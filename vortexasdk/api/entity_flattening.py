...
def _group_vessel_movement_attributes_by_layer(vm: Dict) -> Dict:
    ...
    if 'cargoes' in vm.keys():
        flat_cargoes = [_flatten_attributes(cargo, 'product') for cargo in vm['cargoes']]
...
def _group_cargo_movement_attributes_by_layer(cm: Dict) -> Dict:
    ...
    events = {event_type: list(g) for event_type, g in groupby(cm['events'], lambda x: x['event_type'])}

    events_attributes = {event_type: [_flatten_attributes(ce, 'location') for ce in es] for event_type, es in
                         events.items()}
