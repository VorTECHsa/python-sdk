...
class TestFixturesReal(TestCaseUsingRealAPI):
    ...
    def test_rnd_21448_old_flattener_broken_behavior(self):
        ...
        assert "vessel.corporate_entities.effective_controller.label" not in old_flat or \
               old_flat["vessel.corporate_entities.effective_controller.label"] is None, \
            "RND-21448 (BROKEN): Requesting layer-based corporate_entities returns empty/missing"

        assert "vessel.corporate_entities.time_charterer.label" not in old_flat or \
               old_flat["vessel.corporate_entities.time_charterer.label"] is None, \
            "RND-21448 (BROKEN): Requesting layer-based corporate_entities returns empty/missing"

        # OLD behavior: has numeric indices instead
        old_flat_all = convert_to_flat_dict(first_fixture, columns="all")
        numeric_keys = [k for k in old_flat_all.keys() if "corporate_entities.0." in k or
                        "corporate_entities.1." in k]
    ...
    def test_rnd_21448_new_fixture_flattener_works(self):
        ...
        new_flat = convert_fixture_to_flat_dict(first_fixture_dict, columns="all")

        layer_based_keys = [k for k in new_flat.keys() if "corporate_entities.effective_controller." in k or
                           "corporate_entities.time_charterer." in k or
                           "corporate_entities.charterer." in k]
        assert len(layer_based_keys) > 0, \
    ...
    def test_rnd_21448_to_df_integration(self):
        ...
        has_effective_controller = df["vessel.corporate_entities.effective_controller.label"].notna().any()
        has_time_charterer = df["vessel.corporate_entities.time_charterer.label"].notna().any()
    ...
    def test_to_df_nested_corporate_entities_populated(self):
        ...
        assert len(df) > 0
        assert "vessel.name" in df.columns
        assert "vessel.corporate_entities.effective_controller.label" in df.columns
        assert df["vessel.corporate_entities.effective_controller.label"].notna().any()
